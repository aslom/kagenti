...
|----------|------|------|----------|-----|------------|---------------|:---:|
| `claude_sdk_agent` | Custom A2A | 3 | A2A JSON-RPC | LiteMaaS | No | Via LLM prompt injection | 16/0/2 |
| `adk_agent_supervised` | Custom A2A | **2** | A2A via port-bridge | LiteMaaS | **Yes** | LLM + supervisor security | 17/0/2 |
| `weather_supervised` | Custom A2A | 2 | kubectl exec | No | Yes | N/A (no LLM) | 7/0/14 |
| `openshell_claude` | Builtin sandbox | 1 | kubectl exec | Anthropic/LiteLLM | Yes | Native `.claude/skills/` | 12/0/4 |
| `openshell_opencode` | Builtin sandbox | 1 | kubectl exec | OpenAI-compat | Yes | Via `@ai-sdk/openai-compatible` | 7/0/4 |
| `openshell_generic` | Builtin sandbox | 1 | kubectl exec | N/A | Yes | No agent | 2/0/6 |
| `nemoclaw_hermes` | NemoClaw | 3 | TCP (internal) | LiteMaaS | No | N/A | 10/0/2 |
| `nemoclaw_openclaw` | NemoClaw | 3 | HTTP gateway | LiteMaaS | No | N/A | 17/0/3 |

> **Latest results:** 146 passed, 0 failed, 41 skipped (CI Kind `issue_comment` with LiteMaaS, 2026-05-04)
>
> Per-model tests: `llama-scout-17b` + `deepseek-r1` (both 100% pass rate)

### Capability Coverage Matrix (19 capabilities, 4 tiers)

Every agent must be tested for baseline capabilities. MISS = test doesn't
exist (gap to fill). SKIP = test exists but skipped. FAIL = test ran and failed.

Generated by `.claude/skills/graph-loop/parse-test-matrix.sh --ci-kind`

**Tier 1: Infrastructure (no LLM)**

| # | Capability | Claude Code | OpenCode | Claude SDK | ADK | Weather | OpenClaw |
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| 1 | Connectivity | PASS | PASS | PASS | PASS | PASS | PASS |
| 2 | Credential security | PASS | PASS | PASS | PASS | PASS | PASS |
| 3 | Sandbox lifecycle | PASS | MISS | MISS | MISS | MISS | MISS |
| 4 | Workspace | PASS | PASS | MISS | MISS | MISS | MISS |
| 5 | Resource limits | SKIP | MISS | PASS | SKIP | SKIP | PASS |

**Tier 2: Capabilities (requires LLM)**

| # | Capability | Claude Code | OpenCode | Claude SDK | ADK | Weather | OpenClaw |
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| 6 | Multiturn | SKIP | SKIP | PASS | PASS | SKIP | PASS |
| 7 | Context isolation | SKIP | SKIP | PASS | PASS | SKIP | PASS |
| 8 | Session resume | SKIP | SKIP | SKIP | SKIP | SKIP | MISS |
| 9 | Tool calling | PASS | MISS | MISS | MISS | MISS | MISS |
| 10 | Concurrent sessions | PASS | MISS | MISS | MISS | MISS | MISS |

**Tier 3: Skills (per-model parametrized)**

| # | Capability | Claude Code | OpenCode | Claude SDK | ADK | Weather | OpenClaw |
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| 11 | Skill: PR review | PASS | PASS | PASS | PASS | SKIP | SKIP |
| 12 | Skill: RCA | PASS | PASS | PASS | PASS | SKIP | SKIP |
| 13 | Skill: Security | PASS | PASS | PASS | PASS | SKIP | SKIP |
| 14 | Skill: GitHub PR | PASS | PASS | PASS | PASS | MISS | MISS |

> Per-model parametrization: `llama-scout-17b` + `deepseek-r1` via LiteMaaS (both 100%).
> Local: `qwen3-coder:30b`, `deepseek-r1:32b` via Ollama.
> CI workflows set `OPENSHELL_LLM_MODELS="llama-scout-17b,deepseek-r1"`.

**Tier 4: Security & Policy**

| # | Capability | Claude Code | OpenCode | Claude SDK | ADK | Weather | OpenClaw |
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| 15 | HITL: Network | MISS | MISS | MISS | MISS | PASS | MISS |
| 16 | Audit logging | PASS | PASS | MISS | MISS | MISS | MISS |

**Coverage summary:**

| Agent | Tier 1 | Tier 2 | Tier 3 | Tier 4 | **Total** |
|---|:---:|:---:|:---:|:---:|:---:|
| Claude Code | 4/5 | 2/5 | 4/4 | 1/2 | **11/16** |
| OpenCode | 3/5 | 0/5 | 4/4 | 1/2 | **8/16** |
| Claude SDK | 3/5 | 2/5 | 4/4 | 0/2 | **9/16** |
| ADK | 2/5 | 2/5 | 4/4 | 0/2 | **8/16** |
| Weather | 2/5 | 0/5 | 0/4 | 1/2 | **3/16** |
| OpenClaw | 3/5 | 2/5 | 0/4 | 0/2 | **5/16** |

> Claude Code SKIPs: multiturn/context/session are N/A by design (CLI is single-invocation).
> OpenClaw SKIPs: skills blocked by proprietary gateway protocol (no REST chat API).
> ADK/Weather resource limits: deployment YAMLs don't set limits (not all upstream charts do).
>
...
|----------|-------|--------|--------|---------|-------|
| CI Kind (issue_comment) | 144 | 113 | 1 | 30 | 1 flaky timeout (anthropic passthrough) |
| CI HyperShift | 144 | 78 | 7 | 59 | 7 failures = gateway kube-client (needs PR #1423) |
| Local Kind (iter 4) | 144 | 106 | 1 | 37 | 1 xfail (flaky security review) |
...
- Per-model parametrization via `OPENSHELL_LLM_MODELS` env var
- Ollama local provider via `--llm-provider ollama --llm-models qwen3-coder:30b,deepseek-r1:32b`
