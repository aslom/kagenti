...
```bash
go install github.com/aslom/kagenti/scripts/rossoctl@rossoctlx
```
...
Pin a specific version/commit by replacing `@rossoctlx` with `@v0.1.0` or `@<sha>`.

### Option B — `go run` (no install)

```bash
go run github.com/aslom/kagenti/scripts/rossoctl@rossoctlx version
go run github.com/aslom/kagenti/scripts/rossoctl@rossoctlx doctor
```
